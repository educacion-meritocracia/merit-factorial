...
all.results <- data.frame(matrix(nrow = 3, ncol = 6))
colnames(all.results) <- c("chisq", "df", "p", "rmsea", "cfi", "tli")
rownames(all.results) <- c("Configural", "Thresholds", "Strong")

# Lista de modelos
modelos <- list(fit.baseline, fit.thres, fit.strong)

# Extraer los índices de ajuste
for (i in 1:3) {
  fm <- fitMeasures(modelos[[i]], c("chisq.scaled", "df.scaled", "pvalue.scaled",
                                    "rmsea.scaled", "cfi.scaled", "tli.scaled"))
  all.results[i, ] <- round(unlist(fm), 3)
}

# Mostrar tabla con formato kable
kable(all.results, caption = "Cohort invariance")
```
