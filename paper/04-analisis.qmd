...
fit.baseline <- cfa(model.baseline, data = db1, group = "curse_level",
ordered = c("perc_effort", "perc_talent", "perc_rich_parents", "perc_contact", 
              "pref_effort", "pref_talent", "pref_rich_parents", "pref_contact"))
...
all.results <- data.frame(matrix(nrow = 1, ncol = 6))
colnames(all.results) <- c("chisq", "df", "p", "rmsea", "cfi", "tli")

# Calcular y guardar los índices de ajuste
...
fit.conf <- cfa(model_cfa, data = db1, group = "cohort_level")

fit.metric <- cfa(model_cfa, data = db1, group = "cohort_level",
                  group.equal = c("loadings"))

fit.strong <- cfa(model_cfa, data = db1, group = "cohort_level",
                  group.equal = c("loadings", "intercepts"))

fit.strict <- cfa(model_cfa, data = db1, group = "cohort_level",
...
```{r}
#| label: conf-cohort
#| tbl-cap: "Configural Conditional Invariance"
#| results: asis
#| echo: false
#| warning: false
...
```{r}
#| label: fit-comp-cohort
#| tbl-cap: "Conditional Invariance Test Results"
#| results: asis
#| echo: false
#| warning: false
...
In parallel assess the robustness of longitudinal measurement invariance across time, a series of nested models controlled by cohort effects were added to the analysis. All models exhibited excellent fit, with CFI values above .990 and RMSEA values below .035. The configural model (χ² = 117.70, df = 68, CFI = .991, RMSEA = .035 \[.024–.046\]) served as the baseline. Subsequent models imposing equality constraints on factor loadings (weak invariance), intercepts (strong invariance), and residual variances (strict invariance) all resulted in negligible changes in fit indices (e.g., ΔCFI = .000 across all steps), supporting full longitudinal measurement invariance.

To examine whether these results held when accounting for cohort-related heterogeneity, we re-estimated the models including a dummy variable for cohort as a covariate predicting each latent factor. Fit indices remained excellent (CFI ≥ .990, RMSEA \< .033), and again, the increasingly restrictive models showed minimal changes in model fit: ΔCFI never exceeded .001, and ΔRMSEA values were close to zero or even improved slightly at each step. For example, the strict invariance model with cohort control had χ² = 134.21 (df = 92), CFI = .991, and RMSEA = .028 \[.017–.038\], compared to χ² = 130.17 (df = 84), CFI = .991, and RMSEA = .031 \[.020–.040\] in the model without cohort control.
