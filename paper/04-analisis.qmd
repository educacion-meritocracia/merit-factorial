...
To assess the robustness of longitudinal measurement invariance across time, we first estimated a series of nested models without controlling for cohort effects. All models exhibited excellent fit, with CFI values above .990 and RMSEA values below .035. The configural model (χ² = 117.70, df = 68, CFI = .991, RMSEA = .035 \[.024–.046\]) served as the baseline. Subsequent models imposing equality constraints on factor loadings (weak invariance), intercepts (strong invariance), and residual variances (strict invariance) all resulted in negligible changes in fit indices (e.g., ΔCFI = .000 across all steps), supporting full longitudinal measurement invariance.
