...
A key contribution of this study is its assessment of measurement stability through factorial invariance testing [@putnick_measurement_2016]. We implemented two complementary strategies: longitudinal invariance across two panel waves and cross-group invariance across school levels (primary vs. secondary students). Because the items are ordinal, we followed recent recommendations for ordered categorical indicators, specifically the longitudinal approach of @liu_testing_2017 and the cross-group procedure of @svetina_multiplegroup_2020. In the longitudinal analyses, we estimated the conventional hierarchy of configural, metric, scalar, and strict models, progressively constraining factor loadings, thresholds/intercepts, and residual variances over time. In the cross-group analyses, however, we followed the more appropriate three-step sequence for ordinal indicators proposed by @svetina_multiplegroup_2020, estimating a configural model, then a threshold-invariant model, and finally a model jointly constraining thresholds and factor loadings across groups.
