...
# Overview: Preparation of the EDUMER Students Data Wave        
# Date: 27-12-2025           
#******************************************************************************************************************
...
db_long <- db_long %>% 
  mutate(
    across(
      .cols = -c(id_estudiante,ola,curse_level,p20,p21_ano),
...
frq(db_long$mjp_educ)
frq(db_long$mjp_health)
frq(db_long$mjp_pension)

db_long <- db_long %>% 
  mutate_at(.vars = (14:16),.funs = ~ sjlabelled::set_labels(., labels = labels1))

db_long$mjp_index <- rowMeans(db_long[, c("mjp_educ", "mjp_health", "mjp_pension")], na.rm = TRUE)

frq(db_long$mjp_index)
...
save(db_long, file = here("output/data/db_long_proc2.RData"))
