...
# Anexo {.xlarge data-background-color="#5f5758"}
...
```{r}
#| label: fig-likerplot3
#| fig-align: center
#| fig-cap: "Distribución de respuestas en escala de percepciones y preferencias sobre meritocracia"
...
db_likert1 <- db_long %>%
  filter(cohort_level == "Primary") %>% # quedarse con solo la ola 1
...
likerplot <- e / f + plot_annotation(caption = paste0("Source: Own elaboration based on the EDUMER panel survey"," (n = ",dim(db_likert1)[1],")"
))
...
db_likert1 <- db_long %>%
  filter(cohort_level == "Secondary") %>% # quedarse con solo la ola 1
  select(id_estudiante, starts_with(c("perc", "pref"))) %>% 
  na.omit() %>% 
  mutate(
    across(
...
likerplot <- g / h + plot_annotation(caption = paste0("Source: Own elaboration based on the EDUMER panel survey"," (n = ",dim(db_likert1)[1],")"
))
...
```{r}
#| tbl-cap: "Ajuste para modelos de validez de correlación latente"
#| tbl-cap-location: top
#| echo: false
#| warning: false
