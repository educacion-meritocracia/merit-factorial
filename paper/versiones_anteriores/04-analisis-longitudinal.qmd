...
To identify potential sources of local misfit and assess the robustness of longitudinal measurement invariance, we conducted univariate score tests at each step of the invariance hierarchy—configural, metric (weak), scalar (strong), and strict—using `lavTestScore()` in *lavaan* to evaluate whether specific equality constraints (e.g., loadings, thresholds, residual variances) introduced statistically significant misfit, even when global fit remained acceptable. Across the metric, scalar, and strict models, the score tests indicated no significant violations: loading constraints were supported at the metric level; loadings and thresholds were stable in the scalar model; and residual variances as well as thresholds remained invariant under strict invariance, with the largest test statistic observed for a threshold of the *preference for talent* indicator ($\chi^2$ = 3.829, $p$ = .050) not reaching conventional significance. Taken together, these results provide strong evidence of full longitudinal measurement invariance, supporting meaningful comparisons of latent means, variances, and structural relations across waves; complete score-test outputs are reported in the supplementary materials.
...
Although the analysis found no evidence supporting measurement invariance between cohorts, we nonetheless examined whether plausible cohort-related heterogeneity in socialization processes could affect longitudinal invariance by estimating a series of nested models over time, both with and without controlling for cohort via a dummy covariate predicting each latent factor. Across specifications, model fit consistently exceeded commonly accepted standards, with CFI values above .990 and RMSEA values below .035: the configural baseline model showed $\chi^2$ = 117.70 (df = 68), CFI = .991, and RMSEA = .035 [.024–.046], and subsequent models imposing equality constraints on factor loadings (weak invariance), intercepts (strong invariance), and residual variances (strict invariance) produced negligible changes in fit (e.g., $\Delta$CFI ≤ .001 across all steps). This pattern remained when cohort was included as a covariate; for example, the strict invariance model with cohort control yielded $\chi^2$ = 134.21 (df = 92), CFI = .991, and RMSEA = .028 [.017–.038], closely mirroring the corresponding model without cohort control ($\chi^2$ = 130.17, df = 84, CFI = .991, RMSEA = .031 [.020–.040]). Overall, these results indicate that full longitudinal measurement invariance is maintained over time, regardless of cohort-related heterogeneity.
