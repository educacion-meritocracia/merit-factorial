...
## Possible Error Sources on Longitudinal Invariance

### Weak Invariance

```{r}
#| label: sources-weak
#| tbl-cap: "Possible Error Sources on Weak Invariance"
#| results: asis
#| echo: false
#| warning: false
...
# STEP 1: Run lavTestScore on your model (e.g., weak or strong invariance)
score_result <- lavTestScore(fit_loadinginv)

# STEP 2: Extract univariate test results
score_df <- score_result$uni

# STEP 3: Retrieve parameter table with internal indices
param_tab <- parameterEstimates(fit_loadinginv, standardized = FALSE) %>%
  mutate(index = row_number())

# STEP 4: Extract internal parameter indices from .pX. notation
score_df <- score_df %>%
  mutate(
    lhs_index = as.numeric(str_extract(lhs, "\\d+")),
    rhs_index = as.numeric(str_extract(rhs, "\\d+"))
  )

# STEP 5: Join lhs parameter metadata from parameter table
lhs_info <- param_tab %>%
  select(index, lhs, op, rhs) %>%
  rename(lhs_index = index,
         lhs_var = lhs,
         lhs_op = op,
         lhs_rhs = rhs)

# STEP 6: Join rhs parameter metadata
rhs_info <- param_tab %>%
  select(index, lhs, op, rhs) %>%
  rename(rhs_index = index,
         rhs_var = lhs,
         rhs_op = op,
         rhs_rhs = rhs)

# STEP 7: Merge metadata into score test table
score_annotated <- score_df %>%
  left_join(lhs_info, by = "lhs_index") %>%
  left_join(rhs_info, by = "rhs_index")

# STEP 8: Add readable labels and classify parameter type
score_table <- score_annotated %>%
  mutate(
    parameter_type = case_when(
      lhs_op == "=~" ~ "Loading",
      lhs_op == "~1" ~ "Intercept",
      lhs_op == "~~" ~ "Residual Variance",
      TRUE ~ "Other"
    ),
    restriction = paste0(lhs_var, " ", lhs_op, " ", lhs_rhs,
                         " == ",
                         rhs_var, " ", rhs_op, " ", rhs_rhs),
  ...
  select(restriction, parameter_type, X2, df, p.value, evaluation)

# STEP 9: Create publication-ready table
col.nam <- c("Restriction Tested", "Parameter Type", "Chi-square", "df", "p-value", "Evaluation")
...
```{r}
#| label: sources-strog
#| tbl-cap: "Possible Error Sources on Strong Invariance"
#| results: asis
#| echo: false
#| warning: false
...
# Extract univariate test results
score_df <- score_result$uni

# Retrieve parameter table with internal indices
param_tab <- parameterEstimates(fit_thresholdinv, standardized = FALSE) %>%
  mutate(index = row_number())

# Extract internal parameter indices from .pX. notation
score_df <- score_df %>%
  mutate(
    lhs_index = as.numeric(str_extract(lhs, "\\d+")),
    rhs_index = as.numeric(str_extract(rhs, "\\d+"))
...
# Join lhs parameter metadata from parameter table
lhs_info <- param_tab %>%
  select(index, lhs, op, rhs) %>%
  rename(lhs_index = index,
         lhs_var = lhs,
...
# Merge metadata into score test table
score_annotated <- score_df %>%
  left_join(lhs_info, by = "lhs_index") %>%
  left_join(rhs_info, by = "rhs_index")

# Add readable labels and classify parameter type
score_table <- score_annotated %>%
  mutate(
    parameter_type = case_when(
      lhs_op == "=~" ~ "Loading",
...
#| tbl-cap: "Possible Error Sources of Strict Invariance"
#| results: asis
#| echo: false
#| warning: false
...
# STEP 3: Retrieve parameter table with internal indices
param_tab <- parameterEstimates(fit_uniquenessinv, standardized = FALSE) %>%
  mutate(index = row_number())
...
### Longitudinal invariance for market justice preferences

```{r}
#| tbl-cap: "Longitudinal invariance results for market justice preferences"
#| tbl-cap-location: top
#| echo: false
#| warning: false
#| label: tbl-longinv-mjp
